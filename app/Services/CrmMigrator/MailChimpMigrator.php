...
class MailChimpMigrator extends BaseMigrator
{
    public function getInfo()
    {
        return [
            'title'                  => 'MailChimp',
            'description'            => 'Transfer your mailchimp lists, tags and contacts from MailChimp to FluentCRM',
            ...
            'credential_fields'      => [
                'api_key' => [
                    ...
                    'data_type'   => 'password',
                    'type'        => 'input-text',
                    'inline_help' => 'You can find your API key at MailChimp Account -> Extras -> API keys'
                ]
            ],
    ...
    public function getListTagMappings($postedData)
    {
        ...
        foreach ($lists['lists'] as $list) {
            $formattedLists[] = [
                'id'          => $list['id'],
                'name'        => $list['name'],
                'description' => '(Contacts count ' . Arr::get($list, 'stats.member_count') . ')'
            ];
        }
    ...
    public function getSummary($postedData)
    {
        ...
        $message = 'Based on your selections ' . $count . ' contacts will be imported';
