...
class Bootstrap extends IntegrationManager
{
    ...
    public function getSettingsFields($settings, $formId)
    {
        ...
        if ($paymentFields) {
            ...
            if ($hasSubscriptionFields) {
                $options = [
                    'fluentform_subscription_payment_active'   => 'On Subscription Active',
                    'fluentform_subscription_payment_canceled' => 'On Subscription Cancel',
                    'fluentform_payment_refunded'              => 'On Payment Refund'
                ];
            }
            ...
            $fields[] = [
                'require_list' => false,
                'key'          => 'run_events_only',
                'label'        => 'Run only on events',
                'component'    => 'checkbox-multiple-text',
                'options'      => $options,
                'tips'         => 'If you check any of the events then this feed will only run to the selected events'
            ];
        }
    ...
    private function handlePaymentEvent($submission, $event)
    {
        ...
        $form = fluentFormApi('contacts')->find($submission->form_id);
